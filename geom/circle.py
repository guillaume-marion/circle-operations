...
class Circle(Point):
    ...
    def intersect(self, circle):
        ...
        intersects = self._intersect(
            s_circle_r=s_circle_r,
    ...
    def encompass(self, s_point_or_circle, prec=8):
        ...
        # Test if type is a Point.
            ...
            y = s_point.y
            # Note the smaller than sign; Boundary values will thus return False.
            is_in_circle = ((x - h) ** 2 + (y - k) ** 2).round(prec) < (r**2).round(
                prec
            )
            return is_in_circle
    ...
    def calc_clusters(self):
        ...
        for i, cluster in enumerate(cluster_list):
            ...
            to_remove = [
                cl
                for cl in final_cluster
                if self[list(final_cluster)].encompass(self[int(cl)])
            ]
            ...
            new_clusters_candidates = [
                sett
                for sett in remaining_indices
    ...
    def _all_boundaries(self):
        ...
        for i, intersects_t in enumerate(intersects_l):
            ...
            outer_mask_1 = [
                self.encompass(intersect).any() == False for intersect in intersects_1
            ]
            outer_mask_2 = [
                self.encompass(intersect).any() == False for intersect in intersects_2
            ]
            # Store boundary values based on the encompassment masks
            boundaries_st = Point(
                [intersects_1[outer_mask_1], intersects_2[outer_mask_2]]
            )
            circles_st = self[i]
    ...
    def _orderBoundaries(self, boundaries, inner=False, start_index=None, prec=8):
        ...
        # If no starting index is provided, we make a best guess for a start.
            ...
            start_cindex_l = [
                i for i, _ in enumerate(boundaries_l) if (_.x).round(prec).min() == xmin
            ]
            ...
            boundary = [
                _ for _ in boundaries_l[start_cindex] if _.y.round(prec) == ymin
            ][0]
        ...
        while True:
            ...
            i = [
                i
                for i, _ in enumerate(boundaries_l)
            ...
            if not first and np.all(
                ordered_boundaries_l[0].round(prec) == boundary.round(prec)
            ):
                break
        ...
        try:
            ...
            indices, candidates = zip(
                *[
                    (i, p)
                    for (i, sublist) in enumerate(boundaries_l)
                    for p in sublist
                    if (
                        p.round(prec) != np.array(ordered_boundaries_l).round(prec)
                    ).all()
                ]
            )
    ...
    def calc_boundaries(self, silent=True):
        ...
        if self.iscluster == False:
            raise (
                RuntimeError,
                "Boundaries should be computed for a specific cluster.",
            )
        ...
        indices_combos = [
            (i, j)
            for (i, sublist) in enumerate(boundaries_to_order[0])
        ...
        while True:
            ...
            ordered_boundaries = self._orderBoundaries(
                boundaries_to_order, start_index=selected_index
            )
        ...
        if not silent:
            print(
                "Found a correct outer boundary after {} attempt(s).".format(
                    counter + 2
                )
            )
        # Store the results
        ...
        while len(boundaries_to_order[0]) > 0:
            ...
            while True:
                ...
                # Apply the method.
                ordered_boundaries = self._orderBoundaries(
                    boundaries_to_order, inner=True, start_index=selected_index
                )
                # ...there is actually a result.
                if (
                    ordered_boundaries != None
                ):  # So now the method results None if the angle<270° which is quite hacky and will break in a few cases!!!
                    break
                # If not, we specify another start from the possible starts.
                counter += 1
    ...
    def intersectChord(self, circle):
        ...
        r1 = circle.r
        a = (1 / d) * math.sqrt(
            (-d + r1 - r0) * (-d - r1 + r0) * (-d + r1 + r0) * (d + r1 + r0)
        )
        return a
    ...
    def flatArea(self, return_edge_cases=False):
        ...
        if self.isbounded == False:
            raise (
                RuntimeError,
                "Boundaries should be computed in order to compute flatArea.",
            )
        ...
        if len(self) > 2:
            ...
            distances = [
                float(ordered_cp[i].distance(_)) for i, _ in enumerate(centroids)
            ]  # This should actually be distances to the line-segment and not the centroid!!!
            ...
            circ_segments_A = [
                self._circularSegment(ordered_c[i].r, chords[i])[0]
            ...
            for (
                ordered_b,
                ordered_c,
            ) in inner_boundaries:  # The can be more than 1 hole.
                ...
                # Create the segments from the closed loop boundaries.
                segments = [
                    ordered_all[[i, i + 1]] for i in range(len(ordered_all) - 1)
                ]
                # Compute the circular segments
                chords = [segment[0].distance(segment[1]) for segment in segments]
                circ_segments_A = [
