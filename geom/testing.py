...
for j in range(len(some_cluster)):
    for k in range(len(some_cluster)):
        ...
        for i in indices:
            cplot = plt.Circle(
                (some_cluster[i].x, some_cluster[i].y), some_cluster[i].r, fill=False
            )
            ax.add_artist(cplot)
...
plt.scatter(p[i].x, p[i].y, marker="P", c="black", alpha=0.5, s=250)
...
for p in multip:
    ...
    if isin:
        ...
        plt.scatter(p.x, p.y, color="orange", alpha=0.25)
### Polygon encompass - point per point ###
###########################################
###########################################
...
for i in range(2500):
    ...
    if isin:
        ...
        plt.scatter(multip.x, multip.y, color="orange", alpha=0.25)
### Polygon encompass - multiple points ###
###########################################
...
for i in range(multic.nr_clusters):
    ...
    if len(cluster) == 1:
        c = cluster[0]
        cplot = plt.Circle((c.x, c.y), c.r, color="blue", fill=True, alpha=0.25)
        ax.add_artist(cplot)

    if len(cluster) == 2:
        for c in cluster:
            cplot = plt.Circle((c.x, c.y), c.r, color="orange", fill=True, alpha=0.25)
            ax.add_artist(cplot)

    if len(cluster) > 2:
        # For every cluster...
        for c in cluster:
            cplot = plt.Circle((c.x, c.y), c.r, color="green", fill=True, alpha=0.25)
            ax.add_artist(cplot)
...
ax.set_ylim((0, 35))
for c in mc:
    cplot = plt.Circle((c.x, c.y), c.r, color="blue", fill=False, alpha=0.5)
    ax.add_artist(cplot)
