...
for c in mc:
    cplot = plt.Circle(
        (c.x, c.y), c.r, fill=False, color="gray", alpha=0.7, linewidth=5
    )
    ax.add_artist(cplot)
...
plt.plot(outer_b.x, outer_b.y, color="blue", alpha=0.9, linewidth=3)
...
plt.plot(inner_b.x, inner_b.y, color="green", alpha=0.9, linewidth=3)
